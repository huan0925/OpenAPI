...
def run_composio(msg, Davinci_API_KEY, Composio_API_KEY, Entity_ID):
    # 初始化 OpenAI 客戶端
    openai_client = OpenAI(
        base_url = "https://davinci.mediatek.inc/api/assts/v1",
    ...
    app_enum = getattr(App, 'GMAIL'.upper(), None)
    if app_enum is None:
        raise ValueError(f'Invalid name')
    tool = composio_toolset.get_tools(apps=[app_enum])
...
def home():
    ...
    # print(msg)
    response = run_composio(msg, Davinci_API_KEY, Composio_API_KEY, Entity_ID)
    # print(response)
